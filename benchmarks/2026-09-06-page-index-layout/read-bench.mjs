...
const idColumn = process.env.ID_COLUMN ?? 'fid';
...
async function runOne(path, center, resolution) {
  ...
  try {
    ...
    const bbox = { minX: x - halfWidth, minY: y - halfHeight, maxX: x + halfWidth, maxY: y + halfHeight };
    ...
    const rows = await reader.readRows({
      bbox,
      maxLevel: level,
      columns: [idColumn, geometryColumn],
    });
...
for (const path of files) {
  ...
  for (const resolution of resolutions) {
    for (const center of centers) samples.push(await runOne(path, center, resolution));
  }
  ...
  const readerSample = await runOne(path, centers[0], resolutions[0]);
  result.files[path] = {
    sizeBytes: (await stat(path)).size,
    openBytes: readerSample.openBytes,
    openRequests: readerSample.openRequests,
    byResolution,
    samples,
  };
