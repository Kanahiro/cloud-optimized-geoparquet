...
impl Reader {
    ...
    pub async fn try_new_async<R: AsyncFileReader>(reader: &mut R) -> Result<Self> {
        let arrow_meta = ArrowReaderMetadata::load_async(reader, ArrowReaderOptions::new()).await?;
        Self::from_arrow_metadata(arrow_meta)
    }
    ...
    /// Rendering sidecar root, or `None` when rendering should use
    /// [`Self::primary_column`] directly.
    pub fn lods_column(&self) -> Option<&str> {
        self.cogp_meta.lods_column.as_deref()
    }

    /// Arrow/Parquet column path containing the rendering geometry for a
    /// selected feature level.
    ///
    /// An exact LOD is preferred, followed by the nearest finer stored LOD.
    /// If neither exists (or the sidecar is absent), this returns the primary
    /// geometry column. A coarser LOD is never used for a finer level.
    pub fn rendering_geometry_column(&self, level: usize) -> String {
        let Some(lods_column) = self.lods_column() else {
            return self.primary_column().to_string();
        };
        let Ok(field) = self.arrow_meta.schema().field_with_name(lods_column) else {
            return self.primary_column().to_string();
        };
        ...
        children
            .iter()
            .filter_map(|child| lod_level_index(child.name()))
            .filter(|stored_level| *stored_level >= level)
            .min()
            .map(|stored_level| format!("{lods_column}.level_{stored_level}"))
            .unwrap_or_else(|| self.primary_column().to_string())
    }
    ...
    pub fn row_groups_up_to_resolution(&self, min_resolution: f64) -> Range<usize> {
        let last = self
            ...
            .levels
            .iter()
            .rposition(|level| level.resolution >= min_resolution);
        match last {
