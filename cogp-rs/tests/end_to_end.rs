...
use cogp::meta::{lod_level_index, BboxCovering, Covering, GeoColumn, GeoMeta, GEO_METADATA_KEY};
...
fn write_point_input(path: &std::path::Path) {
    ...
    let geometries: Vec<_> = (0..12)
        .map(|i| wkb::point(f64::from(i), f64::from(i % 3)))
        .collect();
...
fn convert_args(input: &std::path::Path, output: &std::path::Path) -> ConvertArgs {
    ConvertArgs {
        ...
        webmerc_resolution: 4096,
        point_thinning_factor: 16,
        lod_interval: 2,
        sort_key: None,
...
fn convert_reader_validate_pipeline() {
    ...
    assert_eq!(reader.lods_column(), Some("geometry_lods"));
    ...
    // `row_groups_up_to_resolution` returns levels whose resolution is ≥ the target (i.e.
    // coarser than the caller's target). Tiny target → every level
    // qualifies; huge target → no level is that coarse.
    ...
    let lods_field = schema.field_with_name("geometry_lods").unwrap();
    let stored_levels = match lods_field.data_type() {
        DataType::Struct(fields) => fields
            .iter()
            .map(|field| {
                let level_i = lod_level_index(field.name()).expect("canonical LOD child name");
                assert!(level_i < cogp.levels.len() - 1);
                assert_eq!(level_i % 2, 0);
                assert_eq!(field.data_type(), &DataType::Binary);
                assert!(field.is_nullable());
                level_i
            })
            .collect::<Vec<_>>(),
        other => panic!("geometry_lods must be a struct, got {other:?}"),
    };
    assert!(!stored_levels.is_empty());
    assert!(stored_levels.len() < cogp.levels.len());
    assert_eq!(reader.rendering_geometry_column(0), "geometry_lods.level_0");
    if stored_levels.contains(&2) {
        assert_eq!(reader.rendering_geometry_column(1), "geometry_lods.level_2");
    }
    assert_eq!(
        reader.rendering_geometry_column(cogp.levels.len() - 1),
        "geometry"
    );

    // Each stored child is null for rows introduced at a finer level and
    // populated for rows already present at that level.
    for introduced_at in 0..cogp.levels.len() {
        let row_groups: Vec<_> = reader.row_groups_in_level(introduced_at).unwrap().collect();
        let file = File::open(&output).unwrap();
        for batch in reader
            .sync_batch_reader(file, &row_groups)
            .unwrap()
        ...
        {
            let lods = batch
                .column_by_name("geometry_lods")
                .unwrap()
                .as_any()
                .downcast_ref::<StructArray>()
                .unwrap();
            for (column_i, level_i) in stored_levels.iter().copied().enumerate() {
                for row in 0..batch.num_rows() {
                    assert_eq!(lods.column(column_i).is_null(row), level_i < introduced_at);
                }
            }
        }
    }
...
#[test]
fn point_input_uses_primary_wkb_without_lods_column() {
    let tmp = TempDir::new("point-no-lods");
    let input = tmp.path().join("input.parquet");
    ...
    let reader = Reader::open(&output).unwrap();
    assert_eq!(reader.lods_column(), None);
    assert!(reader
        .arrow_metadata()
        .schema()
        .field_with_name("geometry_lods")
        .is_err());
    assert_eq!(reader.primary_column(), "geometry");
}
...
#[test]
fn convert_rejects_zero_lod_interval() {
    let tmp = TempDir::new("zero-lod-interval");
    let input = tmp.path().join("input.parquet");
    let output = tmp.path().join("out.parquet");
    write_input(&input);
    let mut args = convert_args(&input, &output);
    args.lod_interval = 0;
    let err = cogp::convert::run(args).unwrap_err();
    assert!(format!("{err}").contains("--lod-interval"));
}
